...
def prediksi():
    ...
    if 'user_id' in session:
        user = User.query.filter_by(id=session['user_id']).first()
        # Opsional: Jika user tidak ditemukan tapi ada user_id di session, hapus session
        if not user:
            session.pop('user_id', None)
    if request.method == 'POST':
        try:
            ...
            if not (nama and umur and tinggi and berat and jenis_kelamin):
                return render_template('prediksi.html', error="Semua data harus diisi.", hasil_stunting='', risiko_stunting='', user=user)
            ...
            return render_template('prediksi.html', error=f"Error: {e}", hasil_stunting='', risiko_stunting='', user=user)
    return render_template('prediksi.html', hasil_stunting='', risiko_stunting='', error='', user=user)
...
def posyandu():
    ...
    return render_template('posyandu.html', kegiatan=kegiatan, user=user)
...
def kontakkami():
    ...
    return render_template('kontakkami.html', user=user)
